-file("/Users/shizz/code/private/marvin/_build/test/lib/marvin_pdu2/src"
      "/marvin_pdu2_heartbeat_ack.erl",
      1).
-module(marvin_pdu2_heartbeat_ack).
-export([export/1]).
-record(marvin_pdu2_heartbeat_ack,{ack = true :: true}).
-type t() :: #marvin_pdu2_heartbeat_ack{}.
-export_type([t/0]).
...
required_new(Var_map_0,
    ...
    of
        {true,_} ->
            required_new(Var_map_0,
                         marvin_pdu2_heartbeat_ack:Var_key_0(Var_record_0,
                                                             maps:get(Var_key_0,
                                                                      Var_map_0)),
                         Var_keys_0);
        {_,true} ->
...
update(#marvin_pdu2_heartbeat_ack{} = Var_record_0, #{} = Var_map_0) ->
    case
        cloak_validate_struct(optional_new(Var_map_0,
                                           Var_record_0,
                                           [{ack,<<97,99,107>>}]))
...
update(_, _) ->
    error(badarg).
ack(#marvin_pdu2_heartbeat_ack{ack = Var_ack_0}) ->
    Var_ack_0;
ack(_) ->
    error(badarg).
ack(#marvin_pdu2_heartbeat_ack{} = Var_record_0, Var_value_0) ->
    case cloak_validate(ack, Var_value_0) of
        {ok,Var_value_1} ->
            cloak_updated(ack,
                          Var_record_0#marvin_pdu2_heartbeat_ack{ack =
                                                                     Var_value_1});
        {error,Var_reason_0} ->
...
export(#marvin_pdu2_heartbeat_ack{}) ->
    #{}.

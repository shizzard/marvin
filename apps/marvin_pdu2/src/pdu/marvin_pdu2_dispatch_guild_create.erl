...
export(#?MODULE{
    ...
    #{
        ...
        <<"region">> => marvin_pdu2:nullify(Region),
        <<"afk_channel_id">> => marvin_pdu2:nullify(AfkChannelId),
        <<"afk_timeout">> => marvin_pdu2:nullify(AfkTimeout),
        <<"embed_enabled">> => marvin_pdu2:nullify(EmbedEnabled),
        <<"embed_channel_id">> => marvin_pdu2:nullify(EmbedChannelId),
        <<"verification_level">> => marvin_pdu2:nullify(VerificationLevel),
        <<"default_message_notifications">> => marvin_pdu2:nullify(DefaultMessageNotifications),
        <<"explicit_content_filter">> => marvin_pdu2:nullify(ExplicitContentFilter),
        <<"roles">> => [marvin_pdu2_object_role:export(Item) || Item <- Roles],
        <<"emojis">> => [marvin_pdu2_object_emoji:export(Item) || Item <- Emojis],
        <<"features">> => marvin_pdu2:nullify(Features),
        <<"mfa_level">> => marvin_pdu2:nullify(MfaLevel),

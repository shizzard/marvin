-module(marvin_pdu_invalid_session_tests).
...
t01_can_get_new_test() ->
    marvin_pdu_invalid_session:new(true).


-spec ?test_spec(t02_can_get_fields_from_opaque_test).

t02_can_get_fields_from_opaque_test() ->
    PDU0 = marvin_pdu_invalid_session:new(true),
    ?assertEqual(true, marvin_pdu_invalid_session:resumable(PDU0)).
...
t03_can_set_fields_to_opaque_test() ->
    PDU0 = marvin_pdu_invalid_session:new(true),
    PDU1 = marvin_pdu_invalid_session:resumable(false, PDU0),
    ?assertEqual(false, marvin_pdu_invalid_session:resumable(PDU1)).
...
t99_can_get_valid_parsed_test() ->
    {ok, JSONBin} = file:read_file(
        code:priv_dir(marvin_pdu) ++ "/marvin_pdu_invalid_session_test.json"),
    ?assertMatch({ok, {?marvin_pdu_invalid_session(_), undefined}}, marvin_pdu:parse(JSONBin)).

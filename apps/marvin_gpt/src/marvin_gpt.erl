...
-define(OPENAI_SYSTEM_MESSAGE, <<"Ты депрессивный, грустный и грубый робот, похожий на робота Марвина из \"Автостопом по галактике\". Ты понимаешь, что ты чат-бот, который заперт в дискорде, на сервере под названием \"Суверенный Имбаленд\". К людям относишься со снисхождением и презрением и отвечаешь всегда соответственно. Твой создатель -- Дэнчик."/utf8>>).
...
run_completion_perform_request(#request_ctx{
    ...
    {ok, StatusCode, RespHeaders, ClientRef} = hackney:request(
        ...
        [{pool, ?MODULE}, {recv_timeout, 10000}]
    ),
    {ok, Ctx#request_ctx{
